...
const Expense = () => {
    ...
    useEffect(() => {
        ...
        //Загрузка expense-транзакций
        const fetchExpenseTransactions = async () => {
            try {
                const response = await fetch('http://localhost:8000/api/transactions?transaction_type=expense', {
    ...
    return (
        <Box sx={{p: 3, mt: 10, maxWidth: 1200, mx: 'auto'}}>
            ...
            <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3}}>
                <Typography variant="h5">Список расходов</Typography>
                <Button
                    variant="contained"
                    startIcon={<AddIcon/>}
                        ...
                        {filteredAndSortedTransactions.length > 0 ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={5} align="center">
                                    <Typography>Нет расходов</Typography>
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
